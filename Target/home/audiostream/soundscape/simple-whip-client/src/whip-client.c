...
static void on_rtpbin_timeout_cb(GstElement *rtpbin, guint session, guint ssrc, gpointer user_data);
static void connect_rtpbin_timeouts (GstElement *webrtcbin);
...
int main(int argc, char *argv[]) {
	...
	server_url = getenv("DOLBYIO_WHIP_ENDPOINT");
	token = getenv("DOLBYIO_BEARER_TOKEN");
	audio_pipe = getenv("DOLBYIO_AUDIO_PIPE");
	video_pipe = getenv("DOLBYIO_VIDEO_PIPE");
...
static void whip_options(void) {
	...
	guint status = whip_http_send(&session, "OPTIONS", (char *)server_url, NULL, NULL, NULL);
...
static gboolean whip_send_candidates(gpointer user_data) {
	...
	whip_http_session session = { 0 };
	guint status = whip_http_send(&session, "PATCH", resource_url, fragment, "application/trickle-ice-sdpfrag", NULL);
	if(status != 200 && status != 204) {
		/* Couldn't trickle? */
...
static void whip_connect(GstWebRTCSessionDescription *offer) {
	...
	whip_http_session session = { 0 };
	GBytes *bytes = NULL;
	guint status = whip_http_send(&session, "POST", (char *)server_url, sdp_offer, "application/sdp", &bytes);
	g_free(sdp_offer);
	if(status != 201) {
		/* Didn't get the success we were expecting */
	...
	if(bytes == NULL || g_bytes_get_size(bytes) == 0) {
		WHIP_LOG(LOG_ERR, "Missing SDP answer\n");
		g_object_unref(session.msg);
		g_object_unref(session.http_conn);
		if(bytes != NULL)
			g_bytes_unref(bytes);
	...
	if(strstr(answer, "v=0\r\n") != answer) {
		WHIP_LOG(LOG_ERR, "Invalid SDP answer\n");
		g_object_unref(session.msg);
		g_object_unref(session.http_conn);
		whip_disconnect("SDP error");
		return;
...
static void whip_disconnect(char *reason) {
	...
	/* Create an HTTP connection */
	whip_http_session session = { 0 };
	guint status = whip_http_send(&session, "DELETE", resource_url, NULL, NULL, NULL);
	if(status != 200) {
		WHIP_LOG(LOG_WARN, " [%u] %s\n", status, status ? soup_message_get_reason_phrase(session.msg) : "HTTP error");
	}
	g_object_unref(session.msg);
...
static gboolean whip_http_accept_certs(SoupMessage *msg, GTlsCertificate *certificate,
...
static guint whip_http_send(whip_http_session *session, char *method,
	...
	if(error != NULL) {
		WHIP_LOG(LOG_ERR, "Error sending request: %s...\n", error->message);
		g_error_free(error);
		if(rb != NULL)
			g_bytes_unref(rb);
		return 0;
	...
	if(status == 301 || status == 307) {
		...
		if(rb != NULL)
			g_bytes_unref(rb);
		return whip_http_send(session, method, url, payload, content_type, bytes);
	}
...
static void on_rtpbin_timeout_cb (GstElement *rtpbin, guint session, guint ssrc, gpointer user_data)
{
    g_warning ("rtpbin timeout: session=%u ssrc=%u — tearing down", session, ssrc);

    // safest: bounce into your main thread and reuse your existing handler
    g_main_context_invoke (NULL, (GSourceFunc) (void*)whip_handle_signal, (gpointer) (intptr_t)SIGTERM);

    // If whip_handle_signal(int) is thread-safe, you could alternatively call:
    // whip_handle_signal(SIGTERM);
}
...
static void connect_rtpbin_timeouts (GstElement *webrtcbin)
{
	...
	GObject *rtpbin_obj =
    /* webrtcbin implements GstChildProxy; fetch the internal rtpbin */
    rtpbin_obj = gst_child_proxy_get_child_by_name (GST_CHILD_PROXY (webrtcbin), "rtpbin");
    if (!rtpbin_obj) {
        g_error ("Failed to get internal rtpbin from webrtcbin");
        return;
    }
	rtpbin = GST_ELEMENT(rtpbin_obj);

    g_signal_connect (rtpbin, "on-timeout", G_CALLBACK (on_rtpbin_timeout_cb), webrtcbin);
    g_signal_connect (rtpbin, "on-bye-timeout", G_CALLBACK (on_rtpbin_timeout_cb), webrtcbin);
